    ...
    <section className="social-cards-section">
      <div className="social-cards-container">
        <div ref={containerRef} className="fan-layout">
          {cards.map((card, index) => {
            ...
            return card.linkUrl ? (
              <a key={index} href={card.linkUrl} target={card.linkUrl.startsWith("http") ? "_blank" : "_self"} rel="noopener noreferrer" className="fan-card">{image}</a>
            ) : (
      ...
      {needsPagination && (
        <div className="pagination-controls">
          <button className="arrow-btn" onClick={() => cycle("left")} aria-label="Previous">
            {chevron("left")}
          </button>
          <div className="pagination-dots">
            {cards.map((_, i) => (
              <span key={i} className={`pagination-dot ${i === centerIndex ? "active" : "inactive"}`} />
            ))}
          </div>
          <button className="arrow-btn" onClick={() => cycle("right")} aria-label="Next">
            {chevron("right")}
          </button>
        </div>
      )}
